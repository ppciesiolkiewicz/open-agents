...
describe('buildPoolKey', () => {
  it('sorts tokens ascending and derives tickSpacing from feeTier', () => {
    // USDC (0x07...) < UNI (0x8f...)
    const key = buildPoolKey(UNI_ON_UNICHAIN.address, USDC_ON_UNICHAIN.address, 3_000);
    expect(key.currency0.toLowerCase()).toBe(USDC_ON_UNICHAIN.address.toLowerCase());
    expect(key.currency1.toLowerCase()).toBe(UNI_ON_UNICHAIN.address.toLowerCase());
  ...
  it('produces the same key regardless of input order', () => {
    const k1 = buildPoolKey(USDC_ON_UNICHAIN.address, UNI_ON_UNICHAIN.address, 3_000);
    const k2 = buildPoolKey(UNI_ON_UNICHAIN.address, USDC_ON_UNICHAIN.address, 3_000);
    expect(k1).toEqual(k2);
  });

  it('uses tickSpacing 10 for fee 500 and 200 for fee 10000', () => {
    expect(buildPoolKey(USDC_ON_UNICHAIN.address, UNI_ON_UNICHAIN.address, 500).tickSpacing).toBe(10);
    expect(buildPoolKey(USDC_ON_UNICHAIN.address, UNI_ON_UNICHAIN.address, 10_000).tickSpacing).toBe(200);
  });
});
